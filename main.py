...
def setInitialValues():
	"""sets inital variable values"""
	global size, square, vertical, horizontal, boxes, walls, winx, winy, createdBoxes, createdWalls, score, boxColour, menuItems, font, level1, level2, level3, selectedLevel
	size = 600
	score = 0
	square = 40
	vertical = []
...
def reset():
	...
	global me, shoppingcart, score, scoreText, dollarImg
	canvas.delete('all')
	createLines()
...
def createObstacles(min, max): 
	...
	for i in range(amount):
		...
		if [x, y] not in createdBoxes and [x, y] not in createdWalls:
			if x == y == 0: amount += 1; continue
...
def keyPressed(event):
	...
	global score, scoreText, totalBoxes, inmenu
	if not inmenu:
		if score >= totalBoxes:
...
def game(level=None):
	"""Runs game"""
	global me, boxes, walls, img, shoppingcart, score, scoreText, totalBoxes, shoppingcartImg, dollar, dollarImg
